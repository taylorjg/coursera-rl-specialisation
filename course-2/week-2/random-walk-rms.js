...
const doRuns = (predictionMethod, alpha, runs) => {
  ...
  for (const _ of U.rangeIter(runs)) {
...
const main = () => {
  ...
  const line1 = makeLine(td0Values1, 'α=0.1', 'steelblue')
  const line2 = makeLine(td0Values2, 'α=0.15', 'steelblue')
  const line3 = makeLine(td0Values3, 'α=0.05', 'steelblue')
  ...
  const line4 = makeLine(mcValues1, 'α=0.01', 'crimson', { width: 0.5 })
  const line5 = makeLine(mcValues2, 'α=0.02', 'crimson', { dash: 'dash' })
  const line6 = makeLine(mcValues3, 'α=0.03', 'crimson', { dash: 'dot' })
  const line7 = makeLine(mcValues4, 'α=0.04', 'crimson')
  ...
  const layout = {
    ...
    yaxis: {
      title: 'Empirical RMS error, averaged over states',
      range: [0, 0.25]
    },
    annotations: [
